  ...
  record: {
    ...
    post: async (req, res) => {
      if (!req.session.userId) {
        ...
        const data = await record.findOne({ where: { week, day, recordName, userId: req.session.userId } });
        ...
        const total = await record.findAll({where: { userId: req.session.userId }, attributes: ['time']})
        .then(data => {
          let temp = 0;
        ...
        await users_tag.findAll({where: {userId: req.session.userId}, attributes: [sequelize.fn('max', sequelize.col('tagId'))], raw: true})
        .then(data => {
          const curTagId = data[0]['max(`tagId`)'];
          const giveTag = async (tagId) => {
            const isOver = await tag.findOne({where: {id: tagId}, attributes: ['time']})
            .then(data => {
