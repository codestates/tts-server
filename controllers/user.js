  ...
  record: {
    ...
    post: async (req, res) => {
      if (!req.session.userId) {
        ...
        if (!data) {
          ...
          await record.update({time: data.dataValues.time + time}, {where: {week, day, recordName}});
        }
    ...
    get: async (req, res) => {
      if (!req.session.userId) {
        ...
        const userInfo = await user.findOne({where: {id: req.session.userId}});
        const data = await record.findAll({where: {userId: req.session.userId}});
        res.status(200).json({data: data.dataValues, message: userInfo.dataValues.userName + '`s records'});
      }
    }
  },
  // 로그아웃
